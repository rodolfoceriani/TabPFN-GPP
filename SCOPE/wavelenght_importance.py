...
# Recommended: test several values such as 5, 10, 15, 20, 30
N_PLS_COMPONENTS = 20

# Maximum training and testing samples for TabPFN (TabPFN scales quadratically, recommended <= 1000)
MAX_TABPFN_TRAIN_SAMPLES = 14000
MAX_TABPFN_TEST_SAMPLES = 6000
...
# Subsample for TabPFN efficiency (TabPFN scales quadratically, recommended training size <= 1000)
if len(train_df) > MAX_TABPFN_TRAIN_SAMPLES:
    print(f"Subsampling train set from {len(train_df)} to {MAX_TABPFN_TRAIN_SAMPLES} samples for TabPFN...")
    train_df = train_df.sample(n=MAX_TABPFN_TRAIN_SAMPLES, random_state=SEED).reset_index(drop=True)

if len(test_df) > MAX_TABPFN_TEST_SAMPLES:
    print(f"Subsampling test set from {len(test_df)} to {MAX_TABPFN_TEST_SAMPLES} samples for TabPFN...")
    test_df = test_df.sample(n=MAX_TABPFN_TEST_SAMPLES, random_state=SEED).reset_index(drop=True)
...
## ==========================================
## 14. WAVELENGTH IMPORTANCE (TABPFN-AWARE)
## ==========================================

from sklearn.inspection import permutation_importance

print("\nComputing TabPFN-based wavelength importance...")

# ✅ Subsample test set for efficiency (recommended)
MAX_IMPORTANCE_SAMPLES = 2000

X_imp = X_test[:MAX_IMPORTANCE_SAMPLES]
y_imp = y_test[:MAX_IMPORTANCE_SAMPLES]

print(f"Using {len(X_imp)} samples for permutation importance.")
...
perm = permutation_importance(
    ...
    X_imp,
    y_imp,
    n_repeats=3,          # efficient + stable
    random_state=SEED,
    n_jobs=1
)

# ✅ Extract importance of PLS components only
n_other = len(OTHER_COLS)
pls_importance = np.clip(perm.importances_mean[n_other:], a_min=0, a_max=None)

print(f"Computed importance for {len(pls_importance)} PLS components.")

# ✅ Get PLS weights (mapping wavelengths → components)
W = pls.x_weights_   # shape: (n_wavelengths, n_components)

if W.shape[1] != len(pls_importance):
    raise ValueError(
        f"Mismatch: W has {W.shape[1]} components, "
        f"but importance has {len(pls_importance)}."
    )

# ✅ Propagate importance back to wavelengths (robust version)
wave_importance = np.sqrt(
    np.sum((W ** 2) * pls_importance, axis=1)
)

# ✅ Normalize for interpretability
wave_importance = wave_importance / np.max(wave_importance)

# ✅ Save results
importance_df = pd.DataFrame({
    'wavelength': refl_cols,
    'importance': wave_importance
}).sort_values(by='importance', ascending=False)

importance_path = os.path.join(
    script_dir,
    "tabpfn_wavelength_importance.csv"
)

importance_df.to_csv(importance_path, index=False)

print(f"✅ Saved wavelength importance to: {importance_path}")
...
# Extract numeric wavelength from column names (e.g. R_550 → 550)
try:
    wavelengths_numeric = [float(c.split("_")[1]) for c in refl_cols]
except:
    wavelengths_numeric = list(range(len(refl_cols)))  # fallback

plt.plot(wavelengths_numeric, wave_importance, linewidth=1.5)

plt.xlabel("Wavelength (nm)", fontsize=12)
plt.ylabel("Relative importance", fontsize=12)
plt.title(
    "Wavelength Importance (TabPFN + PLS propagation)",
    fontsize=14,
    pad=15
)
...
importance_plot_path = os.path.join(
    script_dir,
    "tabpfn_wavelength_importance.png"
)
